...
class Helpers {
  ...
  public static function enqueue( $script, $handle, $src = '', $deps = array(), $media = 'all', $in_footer = true, $ver = '' ) {
    ...
    $ver = empty($ver) ? filemtime( oact_directory() . $src ) : $ver;

    if ( $script == 'style' ) {

      wp_enqueue_style( esc_attr($handle), oact_directory_uri() . esc_attr($src), $deps, esc_attr($ver), esc_attr($media) );

    } elseif ( $script == 'script' ) {

      wp_enqueue_script( esc_attr($handle), oact_directory_uri() . esc_attr($src), $deps, esc_attr($ver), $in_footer);
  ...
  /**
   * # Get page template URL
   *
   * @param string  $template_path  Path relative to theme
   *
   * @return string URL of the page with that template selected.
   */
  public static function get_page_template_url( $template_path ) {
